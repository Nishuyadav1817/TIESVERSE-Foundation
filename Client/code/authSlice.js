...
// AsyncThunk to send startup data to backend
export const getRecommendation = createAsyncThunk(
  'auth/recommend',
  async (startupData, { rejectWithValue }) => {
    try {
      const response = await axiosClient.post('/user/recommend', startupData);
      return response.data;
     console.log(response.data);
    } catch (error) {
      return rejectWithValue({ message: error.response?.data?.message || 'Recommendation failed' });
    }
  }
);
